...
# argparse : Python Script 실행 시, 하나의 Script 동작을 여러가지 상황에 따라 다르게 동작하도록 할 때 쓰인다. (여러 개 데이터(정해지지 않은 데이터)를 다루고 싶을  <이게 아닐까..)
parser = argparse.ArgumentParser()    #파서(parser) : 원시 프로그램을 읽어 들여, 그문장의 구조를 알아내는 구문 분석(parsing)
...
#DNN은 입력층(input layer)과 출력층(output layer) 사이에 여러 개의 은닉층(hidden layer)들로 이뤄진 인공신경망(Artificial Neural Network, ANN)
...
for idx in range(length):
    ...
    for name, bbox, conf in list_reconized_face:
        ...
        if name == 'unknown':
            ...
            small = cv2.resize(face, None, fx=.07, fy=.07, interpolation=cv2.INTER_NEAREST)        #0.05에서 0.07 -> 숫자 키워야 블러 픽셀 감소
